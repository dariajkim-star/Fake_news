...
1. Monorepo 구조가 생성된다: `src/` 하위에 `data/`, `models/`, `training/`, `evaluation/`, `utils/` 모듈 디렉토리, 최상위에 `configs/`, `scripts/`, `tests/`, `outputs/` 디렉토리가 존재한다 (이후 `detection`/`matching`/`fusion`/`demo` 모듈 확장 가능 구조).
...
- [ ] repo 디렉토리 구조 및 패키지 초기화 (AC: 1)
  - [ ] `src/finfact/` 패키지 생성, 하위 모듈 `data/`, `models/`, `training/`, `evaluation/`, `utils/` 및 `__init__.py`
  - [ ] `configs/`, `scripts/`, `tests/`, `outputs/`(.gitignore 처리) 생성
- [ ] config 시스템 구현 (AC: 2)
  - [ ] yaml 로더 + dataclass/dict 기반 config 객체 (`src/finfact/utils/config.py`)
...
2. 전처리 스크립트가 2-way label(`2_way_label`: 0=fake, 1=real 등 Fakeddit 정의 기준)을 정리하고, 이미지 파일이 존재하고 로딩 가능한(손상 파일 제외) 샘플만 남긴 클린 manifest(csv/parquet: id, image_path, text, label)를 생성한다.
...
- [ ] `FakedditDataset` 구현 (AC: 5, 6)
  - [ ] `src/finfact/data/fakeddit.py` — image transform, tokenizer(max_length, truncation), 모달 선택(text/image/both), subsampling 옵션
...
4. test set에 대해 Accuracy/Precision/Recall/F1/AUROC가 `outputs/<exp_name>/metrics.json`에 기록되고, 두 baseline 모두 random baseline(50%)을 유의미하게 상회한다.
...
- [ ] BERT 분류기 구현 (AC: 1, 5)
  - [ ] `src/finfact/models/text_encoder.py` — `AutoModel` 기반, [CLS](pooled) feature 추출 메서드 + classification head
  - [ ] `configs/bert_only.yaml` (max_length, lr, epochs, batch_size 등)
- [ ] ResNet 분류기 구현 (AC: 2, 5)
  - [ ] `src/finfact/models/image_encoder.py` — torchvision ResNet, fc 교체, pooled feature 추출 메서드
...
- 모델 검증: held-out test set 평가, random baseline 초과 확인, 2회 실행 재현성 확인.
...
- [ ] Late fusion 모델 구현 (AC: 1)
  - [ ] `src/finfact/models/late_fusion.py` — text/image encoder 조합, concat + MLP head (hidden dim, dropout config화)
