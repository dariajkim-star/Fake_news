"""CFTC Enforcement press-release RSS crawler.
"""
...
RSS="https://www.cftc.gov/RSS/RSSENF/rssenf.xml"
# word-boundary regex patterns: plain substring matching let "AI" hit "said"/"email",
# "pump" hit "pumpkin" etc., inflating relevance_score. \w* allows morphological variants.
KEYWORDS=[r"fraud\w*",r"false",r"misleading",r"deceptive",r"social media",r"impersonat\w*",r"\bAI\b",r"deepfake",r"manipulat\w*",r"pump.?and.?dump",r"retail",r"customer\w*",r"victim\w*"]
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--out",default="outputs/cftc.csv");args=ap.parse_args();sess=requests.Session();sess.headers.update(HEADERS)
    ...
    for e in feed.entries:
        ...
        rows.append({"channel":"CFTC","published":e.get("published",""),"title":title,"url":url,"matched_keywords":"|".join(hits),"relevance_score":len(hits),"snippet":body[max(0,pos-200):pos+800] if body else summary[:1000],"body":body[:18000]})
        time.sleep(.2)
